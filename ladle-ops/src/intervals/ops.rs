...
use arrow::array::{
    ArrayRef, Int64Array, Int64Builder, RecordBatch, UInt32Array,
};
...
fn nearest_batches(query: &RecordBatch, target: &RecordBatch) -> Result<RecordBatch, ArrowError> {
    ...
    let t_sorted: HashMap<String, Vec<(i32, i32, u32)>> = t_sorted
        .into_iter()
        .map(|(k, mut v)| { v.sort_unstable_by_key(|&(s, _, _)| s); (k, v) })
        .collect();
        ...
        .flat_map(|(chrom, q_rows)| -> Vec<(u32, Option<u32>, Option<i64>)> {
            ...
            q_rows.iter().map(|&q_idx| {
                ...
                let mut update = |dist: i64, t_idx: u32| {
                    if best.map_or(true, |(d, bi)| dist < d || (dist == d && t_idx < bi)) {
    ...
    for col_idx in 0..n_b_cols {
        ...
        let idx_arr: Int64Array = matched_b
            .iter()
            .map(|&t| t.map(|v| v as i64))
            .collect();
...
fn count_overlaps_batches(a: &RecordBatch, b: &RecordBatch) -> Result<RecordBatch, ArrowError> {
    ...
    let mut fields: Vec<arrow::datatypes::Field> = a
        ...
        .map(|f| f.as_ref().clone())
        .collect();
    fields.push(arrow::datatypes::Field::new("count", arrow::datatypes::DataType::UInt32, false));
...
fn cluster_batches(batch: &RecordBatch) -> Result<RecordBatch, ArrowError> {
    ...
    for (chrom, start, end, orig_idx) in &sorted {
        if *chrom != cur_chrom {
            if !cur_chrom.is_empty() { cluster_id += 1; }
...
fn subtract_batches(a: &RecordBatch, b: &RecordBatch) -> Result<RecordBatch, ArrowError> {
    ...
    for row in 0..b.num_rows() {
        if let Some((chrom, start, end)) = get_interval(b, &b_cols, row) {
            b_groups.entry(chrom.to_string()).or_default()
                .push(Interval::new(start, end - 1, row as u32));
        }
    }
    let trees: HashMap<String, COITree<u32, u32>> = b_groups.into_iter()
        .map(|(k, v)| (k, COITree::new(&v)))
        .collect();
...
fn complement_batches(
    ...
    for (chrom, start, end, _) in &sorted {
        if *chrom != cur_chrom {
            ...
            if let Some(sizes) = chrom_sizes {
                if let Some(&sz) = sizes.get(&cur_chrom) {
                    if cur_end < sz { gaps.push((cur_chrom.clone(), cur_end, sz)); }
                }
            }
            cur_chrom = chrom.clone();
            cur_end = 0;
    ...
    if let Some(sizes) = chrom_sizes {
        if let Some(&sz) = sizes.get(&cur_chrom) {
            if cur_end < sz { gaps.push((cur_chrom, cur_end, sz)); }
        }
    }
...
fn expand_batches(
    ...
    let old_start = batch.column(cols.start).as_any().downcast_ref::<Int32Array>()
        .ok_or_else(|| ArrowError::InvalidArgumentError("start column is not Int32".into()))?;
    let old_end = batch.column(cols.end).as_any().downcast_ref::<Int32Array>()
...
fn sort_bedframe_batches(batch: &RecordBatch, natural: bool) -> Result<RecordBatch, ArrowError> {
    ...
    let sort_indices = if natural {
        ...
        let chrom_col = batch.column(cols.chrom).as_any()
        ...
        let start_col = batch.column(cols.start).as_any().downcast_ref::<Int32Array>()
        ...
        lexsort_to_indices(&[
            ...
            SortColumn { values: Arc::new(start_col.clone()) as ArrayRef, options: Some(opts) },
        ], None)?
    } else {
        lexsort_to_indices(&[
    ...
    let columns: Result<Vec<ArrayRef>, _> = batch.columns().iter()
...
fn flank_batches(batch: &RecordBatch, width: i32, start: bool) -> Result<RecordBatch, ArrowError> {
    ...
    let (new_start, new_end): (Int32Array, Int32Array) = if start {
        ...
        let ns = (0..batch.num_rows()).map(|i| (old_start.value(i) - width).max(1)).collect();
        ...
        let ne = (0..batch.num_rows()).map(|i| old_end.value(i) + width).collect();
        (ns, ne)
    };
...
fn set_width_batches(batch: &RecordBatch, width: i32, anchor: &str) -> Result<RecordBatch, ArrowError> {
    use arrow::array::Int32Array;
    ...
    let (new_start, new_end): (Int32Array, Int32Array) = match anchor {
        "start" => {
            ...
            let ne = (0..batch.num_rows()).map(|i| old_start.value(i) + width).collect();
            (ns, ne)
        }
        "end" => {
            let ns = (0..batch.num_rows()).map(|i| (old_end.value(i) - width).max(1)).collect();
            let ne = (0..batch.num_rows()).map(|i| old_end.value(i)).collect();
            (ns, ne)
        }
        "center" => {
            let ns = (0..batch.num_rows()).map(|i| {
                let mid = (old_start.value(i) + old_end.value(i)) / 2;
                (mid - width / 2).max(1)
            }).collect();
            let ne = (0..batch.num_rows()).map(|i| {
                let mid = (old_start.value(i) + old_end.value(i)) / 2;
                mid + (width + 1) / 2
            }).collect();
            (ns, ne)
        }
        other => return Err(ArrowError::InvalidArgumentError(
            format!("anchor must be 'start', 'end', or 'center'; got '{}'", other)
        )),
    };
...
fn disjoin_batches(batch: &RecordBatch) -> Result<RecordBatch, ArrowError> {
    ...
    while i < sorted.len() {
        ...
        let j = sorted[i..].iter().position(|(c, _, _, _)| c != &chrom)
            .map(|p| i + p)
        ...
        for w in pts.windows(2) {
            ...
            let covered = sorted[i..j].iter().any(|(_, s, e, _)| *s <= bp_s && *e >= bp_e);
            if covered {
...
fn intersect_ranges_batches(a: &RecordBatch, b: &RecordBatch) -> Result<RecordBatch, ArrowError> {
    ...
    for (chrom, start, end) in raw {
        match merged.last_mut() {
            Some(last) if last.0 == chrom && start < last.2 => { last.2 = last.2.max(end); }
            _ => merged.push((chrom, start, end)),
        }
    }
    chrom_start_end_batch(merged)
...
pub fn py_cluster<'py>(py: Python<'py>, a: &Bound<'_, PyAny>) -> PyResult<Bound<'py, PyAny>> {
    ...
    let result = py.detach(|| cluster_batches(&batch)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_merge<'py>(py: Python<'py>, a: &Bound<'_, PyAny>) -> PyResult<Bound<'py, PyAny>> {
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| merge_batches(&batch)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_subtract<'py>(
    ...
    let batch_a = pyarrow_to_batch(py, a)?;
    let batch_b = pyarrow_to_batch(py, b)?;
    let result = py.detach(|| subtract_batches(&batch_a, &batch_b)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_complement<'py>(
    ...
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| complement_batches(&batch, chrom_sizes.as_ref()))
        .map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_coverage<'py>(py: Python<'py>, a: &Bound<'_, PyAny>) -> PyResult<Bound<'py, PyAny>> {
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| coverage_batches(&batch)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_expand<'py>(
    ...
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| expand_batches(&batch, amount, start_amount, end_amount))
        .map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}

#[pyfunction(name = "shift")]
pub fn py_shift<'py>(py: Python<'py>, a: &Bound<'_, PyAny>, amount: i32) -> PyResult<Bound<'py, PyAny>> {
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| shift_batches(&batch, amount)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_sort_bedframe<'py>(
    ...
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| sort_bedframe_batches(&batch, natural_chrom_order))
        .map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_flank<'py>(
    ...
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| flank_batches(&batch, width, start)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_set_width<'py>(
    ...
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| set_width_batches(&batch, width, anchor))
        .map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}

#[pyfunction(name = "tile")]
pub fn py_tile<'py>(py: Python<'py>, a: &Bound<'_, PyAny>, width: i32) -> PyResult<Bound<'py, PyAny>> {
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| tile_batches(&batch, width)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_disjoin<'py>(py: Python<'py>, a: &Bound<'_, PyAny>) -> PyResult<Bound<'py, PyAny>> {
    let batch = pyarrow_to_batch(py, a)?;
    let result = py.detach(|| disjoin_batches(&batch)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_intersect_ranges<'py>(
    ...
    let result = py.detach(|| intersect_ranges_batches(&ba, &bb)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_union_ranges<'py>(
    ...
    let ba = pyarrow_to_batch(py, a)?;
    let bb = pyarrow_to_batch(py, b)?;
    let result = py.detach(|| union_ranges_batches(&ba, &bb)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
...
pub fn py_setdiff_ranges<'py>(
    ...
    let ba = pyarrow_to_batch(py, a)?;
    let bb = pyarrow_to_batch(py, b)?;
    let result = py.detach(|| setdiff_ranges_batches(&ba, &bb)).map_err(|e| PyIOError::new_err(e.to_string()))?;
    batch_to_pyarrow(py, result)
}
