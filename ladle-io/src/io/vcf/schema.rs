...
pub fn build_vcf_batch_with_header(
    ...
    for rec in records {
        ...
        if n_samples > 0 && !fmt_keys.is_empty() {
            ...
            let mut fmt_values: Vec<Vec<Option<String>>> = fmt_keys
                .iter()
            ...
            for (fmt_idx, vals) in fmt_values.iter().enumerate() {
                if vals.is_empty() {
                    ...
                    let joined: Vec<&str> =
                        vals.iter().map(|v| v.as_deref().unwrap_or(".")).collect();
...
pub fn sample_value_to_string(
    v: noodles::vcf::variant::record::samples::series::Value<'_>,
) -> String {
