...
use super::batch::PySamRecordBatch;
...
impl PyReader {
    ...
    fn records_to_batch(&mut self, py: Python<'_>) -> PyResult<PySamRecordBatch> {
        let reader = self.get()?;
        let mut records: Vec<noodles::sam::Record> = Vec::new();
        let mut record = noodles::sam::Record::default();
        py.detach(|| -> PyResult<()> {
            loop {
                let n = reader
        ...
        PySamRecordBatch::try_new(records).map_err(|e| PyIOError::new_err(e.to_string()))
    }
